...
The `vcard` module (behind `rfc-6350`/`rfc-2426`) is a module of its own, not part of the iCalendar model. The rules above apply to it with "the module's own" files: vCard properties hold only values from `src/vcard/values.rs` and params from `src/vcard/params.rs`, never iCalendar-specific types from the top-level `values.rs`/`params.rs`. The primitives whose grammar is identical (`Text`, `Uri`, `Binary`, `MediaType`) may be reused from the top-level `values.rs` and re-exposed through `vcard/values.rs`, so vCard properties still reference values from one place. Same-named types with different semantics (`ValueDataType`, `Uid`, date/time, and also `Boolean` (case-insensitive), `Integer` (64-bit), `Float` (no exponent) and `UtcOffset` (`+hh` allowed)) are defined separately in `vcard/`, never shared.
